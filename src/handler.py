...
from utilidades import pagos_usuario, detalle_individual_historico, detalle_individual_puntos, poner_fondo_gris, crear_tabla_quinielas, crear_tabla_general, crear_tabla_resultados
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    with INFORMACION_BOTS[context.bot_data['nombre']]['sesion']()  as sesion:
        ...
        if not usuario:
            ...
            sesion.commit()
        texto =  INFORMACION_BOTS[context.bot_data['nombre']]['textostart']
        await update.message.reply_text(
            texto, 
            reply_markup=ReplyKeyboardRemove()
        )
...
async def mihistorico(update:Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    with INFORMACION_BOTS[context.bot_data['nombre']]['sesion']() as sesion:
        im, mensaje = detalle_individual_historico(sesion=sesion, telegram_id=update.message.from_user.id, bot_nombre=context.bot_data['nombre'])
...
async def mispuntos(update:Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    with INFORMACION_BOTS[context.bot_data['nombre']]['sesion']() as sesion:
        im, mensaje = detalle_individual_puntos(sesion=sesion, telegram_id=update.message.from_user.id, bot_nombre=context.bot_data['nombre'])
    if mensaje == 'No hay carreras archivadas.':
        await update.message.reply_text(mensaje)
        return ConversationHandler.END
...
async def resultados(update:Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    with INFORMACION_BOTS[context.bot_data['nombre']]['sesion']() as sesion:
        im, texto = crear_tabla_resultados(sesion, None, context.bot_data['nombre'])
...
async def inicio_pilotos(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    Piloto = INFORMACION_BOTS[context.bot_data['nombre']]['tablas']['piloto']
    Carrera = INFORMACION_BOTS[context.bot_data['nombre']]['tablas']['carrera']
    ...
    if(ahora > horario_qualy):
        ...
        return ConversationHandler.END
    pilotos.sort(key=lambda x: x.acumulado_puntos, reverse=True)
    COLUMNAS = 5
    keyboard = []
    cuenta_columnas = COLUMNAS
...
async def guardar_pilotos(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    Usuario = INFORMACION_BOTS[context.bot_data['nombre']]['tablas']['usuario']
    Quiniela = INFORMACION_BOTS[context.bot_data['nombre']]['tablas']['quiniela']
    with INFORMACION_BOTS[context.bot_data['nombre']]['sesion']() as sesion:
        query = update.callback_query
        await query.answer()
        carrera_quiniela = context.user_data['NombreCarrera']
